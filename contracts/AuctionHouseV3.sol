...
contract AuctionHouse is Ownable, Pausable, ReentrancyGuard {
	...
	function createAuction(
		...
		if (isERC721(nftContract)) {
			...
			auctionToken.safeTransferFrom(msg.sender, address(this), nftID, 1, "");
		} else {
		...
		emit AuctionListed(al.auctionId, msg.sender, nftContract, al.startPrice, tick, al.endTime);
	...
	function claim(uint256 auctionId) external nonReentrant {
		...
		if (al.bidCount == 0) {
			...
			if (isERC721(al.nftContract)) {
				...
				auctionToken.safeTransferFrom(address(this), al.auctioneer, auctions[auctionId].nftID);
	...
	function putForSale(
		...
		require(saleDurationInSeconds <= maxSaleTime, "sale time > maxSaleTime");
		...
		if (isERC721(nftContract)) {
			...
			revert("only ERC721 and ERC1155 types of tokens are supported for sale");
		}
		...
		emit OnSale(nftContract, nftID, price, block.timestamp + saleDurationInSeconds);
	}
	...
	function removeFromSale(uint256 saleID) external {
		...
		if (isERC721(sl.nftContract)) {
			...
			auctionToken.safeTransferFrom(address(this), msg.sender, sl.nftID, 1, "");
		}
		emit RemovedFromSale(saleID, sl.nftID);
	}
