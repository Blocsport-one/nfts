...
contract BlocsportNFTERC155WithToken is ERC1155, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;
	using SafeMath for uint256;
	using Strings for string;

    // blocsport token
    IERC20 public immutable blsToken;

	//priceSetter can set the price & maxSupply for an NFT
	address public priceSetter;

	string public _baseURI = "https://nft-storage-system.s3.eu-west-1.amazonaws.com/nfts-2/bsc/metadata/";
    ...
    mapping(uint256 => uint256) public priceInBLS;

	constructor(address _blsToken) ERC1155(_baseURI) {
        require(_blsToken != address(0), "address cannot be null");

        blsToken = IERC20(_blsToken);
    ...
    /**
    * @dev buy nft using BLS tokens 
    * @param _id token id 
    * @param _amount amount in bls token
    */
	function buyNFTWithBLS(uint256 _id, uint256 _amount) external nonReentrant {
		require(priceInBLS[_id] != 0, "price not set");
        require(priceInBLS[_id] == _amount, "incorrect amount");
		require(totalSupply[_id] < maxSupply[_id], "max quantity reached");
